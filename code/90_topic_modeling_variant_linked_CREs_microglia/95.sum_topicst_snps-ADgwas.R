...
# AD GWAS pval
path_gwas <- "/geschwindlabshares/RexachGroup/Xia_Data/resources/GWAS/GWAS_resource"

AD_gwas_bell <- read.csv(file.path(path_gwas, "Raw.AD_Bellenguez.tsv"), sep = "\t")
rownames(AD_gwas_bell) <- AD_gwas_bell$SNP

AD_gwas_bell[AD_gwas_bell$SNP %in% "rs10001872", ]
...
for (topic in topics){
  ...
  with_gwas <- AD_gwas_bell[AD_gwas_bell$SNP %in% dt_cres_w_snp$snp, ]
  rownames(with_gwas) <- with_gwas$SNP
  with_gwas <- with_gwas[dt_cres_w_snp$snp, ]
  dt_cres_w_snp$gwas_pval <- with_gwas$p_value
  dt_cres_w_snp$gwas_beta <- with_gwas$odds_ratio
...
p2 <- ggplot(dt_show, aes(x = topic, y = -log10(pval), colour = topic)) +
  ...
  scale_colour_manual(values=colors.topics) +
  geom_hline(yintercept = -log10(5e-8), color = "red", linetype = "dotted") +
  labs(x = "", y = "-log10(AD Bellenguez GWAS p-value)",
...
pdf(file.path(outp, paste("pDot.topics_SNPs.DynCRE.ADgwas.pdf",sep = ".")), height = 4)
p1
p2
p3
dev.off()
...
for (topic in topics){
  ...
  p_scatter <- ggplot(dtp, aes(x = -log10(pval), y = -log10(gwas_pval), colour = topic)) +
    ...
    labs(x = "-log10(eQTL/MPRA p-value)", y = "-log10(AD GWAS p-value)",
...
pdf(file.path(outp, paste("pDot.PxP_per_topic.DynCRE.ADgwas.pdf",sep = ".")), height = 3, width = 6)
ps
dev.off()
...
write.csv(combined, file.path(outp, "table_topics_snps_TF.ADgwas.csv"))
...
pv <- ggplot(dt_fisher, aes(x = log2(OR), y = -log10(fdr), label = topic)) +
  ...
  labs(title = "Topics enriched in SNPs by Fisher test",
       ...
       y = "-log10(FDR)") +
  add
pdf(file.path(outp, paste("pDot.fisher_topic_w_snps.DynCRE.ADgwas.pdf",sep = ".")), height = 3, width = 4)
pv
dev.off()
